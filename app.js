...
for (let i = 0; i < obj.length; i++) {
    ...
    add.innerHTML += `<div id ="brand">
            ...
            <h5>${obj[i].rating.rate}</h5>
             <button id="btn" data-index="${i}">Add to Cart</button>
...
select.addEventListener('change', () => {
    ...
    for (let i = 0; i < obj.length; i++) {
        if (selectedCategory == 'All Products' || selectedCategory == obj[i].category) {
            add.innerHTML += `
                    <div id ="brand">
            <img src="${obj[i].image}" alt="">
            <h1>${obj[i].title}</h1>
...
function attachCartEventListeners() {
    ...
    for (let i = 0; i < btn.length; i++) {
        btn[i].addEventListener('click', (event) => {
            const productIndex = event.target.getAttribute('obj-index');
            Swal.fire({
                title: "Are you sure?",
